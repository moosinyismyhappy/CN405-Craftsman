...
def my_timer():
    ...
    global input1_timer_status, input1_start_time, input1_end_time, input1_total_time, input1_is_pass_interested_area, input1_counter, input1_in_round_time
    global input2_timer_status, input2_start_time, input2_end_time, input2_total_time, input2_is_pass_interested_area, input2_counter, input2_in_round_time
    global output_timer_status, output_start_time, output_end_time, output_total_time, output_is_pass_interested_area, output_counter, output_in_round_time
    global work_timer_status, work_start_time, work_end_time, work_total_time, work_counter, work_in_round_time
    global cycle_start_time, cycle_end_time, cycle_timer_status, cycle_total_time, cycle_is_pass_interested_area, cycle_counter, cycle_in_round_time
    ...
    while True:
        ...
        if timer_button is True:
            ...
            if current_area_left == 4:
                ...
                if input1_timer_status == -1:
                    ...
                    input1_in_round_time = round((input1_end_time - input1_start_time), 2)
                    # print('Input1 Time:', input1_in_round_time, 'seconds')
                    # add in round time to total time and increase counter by 1 for finish round
                    input1_total_time = input1_total_time + input1_in_round_time
                    input1_counter = input1_counter + 1
            ...
            # center is in work area with timer start and already pass input2 area
                ...
                input2_in_round_time = round((input2_end_time - input2_start_time), 2)
                # print('Input2 Time:', input2_in_round_time, 'seconds')
                # add in round time to total time and increase counter by 1 for finish round
                input2_total_time = input2_total_time + input2_in_round_time
                input2_counter = input2_counter + 1
            ...
            # center is out of output area. stop timer and calculate used time
                ...
                if (output_end_time - output_start_time) == 0:
                    ...
                    output_in_round_time = round((output_end_time - output_start_time), 2)
                    # print('Output Time:', output_in_round_time, 'seconds')
                    # add in round time to total time and increase counter by 1 for finish round
                    output_total_time = output_total_time + output_in_round_time
            ...
            # center is out of work area. stop timer and calculate used time
                ...
                work_in_round_time = round((work_end_time - work_start_time), 2)
                # print('Work Time:', work_in_round_time, 'seconds')
                # add in round time to total time and increase counter by 1 for finish round
                work_total_time = work_total_time + work_in_round_time
            ...
            if current_area_right == 3:
                ...
                # stop timer and calculate used time
                    ...
                    cycle_in_round_time = round((cycle_end_time - cycle_start_time), 2)
                    # print('Cycle Time:', cycle_in_round_time, 'seconds')
                    # add in round time to total time and increase counter by 1 for finish round
                    cycle_total_time = cycle_total_time + cycle_in_round_time
        ...
        # display each round time
        if not input1_in_round_time < 0:
            if timer_button is False:
                cv2.putText(image_frame, 'Stop', (10, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (0, 0, 255), 2)
            else:
                cv2.putText(image_frame, str(input1_in_round_time) + ' seconds', (10, 400), cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (0, 0, 255), 2)

        if not input2_in_round_time < 0:
            if timer_button is False:
                cv2.putText(image_frame, 'Stop', (140, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (255, 255, 255), 2)
            else:
                cv2.putText(image_frame, str(input2_in_round_time) + ' seconds', (140, 400), cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (255, 255, 255), 2)

        if not output_in_round_time < 0:
            if timer_button is False:
                cv2.putText(image_frame, 'Stop', (270, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (0, 80, 255), 2)
            else:
                cv2.putText(image_frame, str(output_in_round_time) + ' seconds', (270, 400), cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (0, 80, 255), 2)

        if not work_in_round_time < 0:
            if timer_button is False:
                cv2.putText(image_frame, 'Stop', (400, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (150, 80, 255), 2)
            else:
                cv2.putText(image_frame, str(work_in_round_time) + ' seconds', (400, 400), cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (150, 80, 255), 2)

        if not cycle_in_round_time < 0:
            if timer_button is False:
                cv2.putText(image_frame, 'Stop', (520, 400), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (145, 10, 91), 2)
            else:
                cv2.putText(image_frame, str(cycle_in_round_time) + ' seconds', (520, 400), cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (145, 10, 91), 2)
        ...
        if input1_counter == 0:
            ...
            input1_average_time = input1_total_time / input1_counter
            input1_average_time = round(input1_average_time, 2)
            cv2.putText(image_frame, str(input1_average_time) + ' seconds', (10, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 0, 255), 2)
        # input2 time
        if input2_counter == 0:
            ...
            input2_average_time = input2_total_time / input2_counter
            input2_average_time = round(input2_average_time, 2)
            cv2.putText(image_frame, str(input2_average_time) + ' seconds', (140, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (255, 255, 255), 2)
        # output time
        if output_counter == 0:
            ...
            output_average_time = output_total_time / output_counter
            output_average_time = round(output_average_time, 2)
            cv2.putText(image_frame, str(output_average_time) + ' seconds', (270, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 80, 255), 2)
        # work time
        if work_counter == 0:
            ...
            work_average_time = work_total_time / work_counter
            work_average_time = round(work_average_time, 2)
            cv2.putText(image_frame, str(work_average_time) + ' seconds', (400, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (150, 80, 255), 2)
        # cycle time
        if cycle_counter == 0:
            ...
            cycle_average_time = cycle_total_time / cycle_counter
            cycle_average_time = round(cycle_average_time, 2)
            cv2.putText(image_frame, str(cycle_average_time) + ' seconds', (520, 450), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,(145, 10, 91), 2)
